...
namespace eutelescope {
   ...
   *  EUTelAnnulusPixel can be easily cast to an
...
public:
    ...
    EUTelAnnulusPixel(short xCoord, short yCoord, float signal, short time, float posX, float posY, float rmin, float rmax, float dphi, float rlength, float angle);

    //! Default constructor with all arguments (EUTelGenericSparsePixel, 4 additional geometry related)
    EUTelAnnulusPixel(EUTelGenericSparsePixel& genericPixel, float posX, float posY, float rmin, float rmax, float dphi, float rlength, float angle); 

    //! Default constructor with only arguments of a EUTelescopeGenericSparsePixel (geometry related values are set to 0)
    EUTelAnnulusPixel(EUTelGenericSparsePixel& genericPixel); 
    ...
    void setang(float angle) { _ang = angle; }
    ...
    inline float getang() const { return _ang; }
